...
(function() {
  ...
  } else if ("***REMOVED***" in window) { // Google charts
    // load from ***REMOVED***
    var loaded = false;
    ***REMOVED***.setOnLoadCallback(function() {
      loaded = true;
    });
    ***REMOVED***.load("visualization", "1.0", {"packages": ["corechart"]});

    var waitForLoaded = function(callback) {
      ***REMOVED***.setOnLoadCallback(callback); // always do this to prevent race conditions (watch out for other issues due to this)
      if (loaded) {
        callback();
      }
    };
    ...
    var createDataTable = function(series, columnType) {
      var data = new ***REMOVED***.visualization.DataTable();
    ...
    renderLineChart = function(element, series, opts) {
      waitForLoaded(function() {
        ...
        var chart = new ***REMOVED***.visualization.LineChart(element);
    ...
    renderPieChart = function(element, series, opts) {
      waitForLoaded(function() {
        ...
        var chart = new ***REMOVED***.visualization.PieChart(element);
        chart.draw(data, options);
      });
    };
    ...
    renderColumnChart = function(element, series, opts) {
      waitForLoaded(function() {
        ...
        var chart = new ***REMOVED***.visualization.ColumnChart(element);
        chart.draw(data, options);
      });
    };
